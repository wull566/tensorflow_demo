...
tf_y = tf.placeholder(tf.float32, [None, TIME_STEP, OUTPUT_SIZE])          # input y

# RNN

# ins2D = tf.reshape(tf_x, [-1, INPUT_SIZE])                       # reshape 3D output to 2D for fully connected layer
# net_ins2D = tf.layers.dense(ins2D, CELL_SIZE)
# ins = tf.reshape(net_ins2D, [-1, TIME_STEP, CELL_SIZE])          # reshape back to 3D
...
net_outs2D = tf.layers.dense(outs2D, OUTPUT_SIZE)
outs = tf.reshape(net_outs2D, [-1, TIME_STEP, OUTPUT_SIZE])          # reshape back to 3D

loss = tf.losses.mean_squared_error(labels=tf_y, predictions=outs)  # compute cost 均方差误差
...
for step in range(200):
    ...
    _, pred_, final_s_, loss_ = sess.run([train_op, outs, final_s, loss], feed_dict)     # train

    # plotting
    plt.plot(steps, y.flatten(), 'r'); plt.plot(steps, pred_.flatten(), 'b--')
    plt.ylim((-1.2, 1.2)); plt.draw(); plt.pause(0.3)

    if step % 10 == 0:
        print('loss: ', round(loss_, 4))
