...
# Hyper Parameters
BATCH_SIZE = 64
LR_G = 0.0001           # 画家 (生成器) 1_tensorflow_new rate for generator
LR_D = 0.0001           # 鉴赏家 (判别器) 1_tensorflow_new rate for discriminator
...
def artist_works():     # painting from the famous artist (real target)
    ...
    paintings = a * np.power(PAINT_POINTS, 2) + (a-1)   # a * x^2 + (a-1)
    return paintings
...
# 尽量增加 著名画家prob_artist0概率， 尽量减少新手画家prob_artist1 概率
D_loss = -tf.reduce_mean(tf.log(prob_artist0) + tf.log(1-prob_artist1))
# 新手画家的画，尽量减少认为是新手画家prob_artist1的概率
...
for step in range(5000):
    ...
    G_ideas = np.random.randn(BATCH_SIZE, N_IDEAS)  # 随机生成y轴的点
