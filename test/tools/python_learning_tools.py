...
def check_version():
    ...
    if v.major == 3 and v.minor >= 5:
        return
    print('Your current 3_python is %d.%d. Please use Python 3.6.' % (v.major, v.minor))
    exit(1)

def check_cert():
    today = datetime.now().strftime('%Y-%m-%d')
    if today >= CERT_EXPIRES:
        print('This 1_tensorflow_new.py is expired. Please download a newer version.')
        exit(1)
